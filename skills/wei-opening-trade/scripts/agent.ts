...
/** Judge prompt template (opening trade) */
const JUDGE_OPENING_TRADE_PROMPT_TEMPLATE = loadPrompt('judge_opening_trade');
...
`).join('\n---\n');
    ...
    const template = domain === 'financial'
      ? JUDGE_FINANCIAL_PROMPT_TEMPLATE
      : domain === 'opening-trade'
        ? JUDGE_OPENING_TRADE_PROMPT_TEMPLATE
